...
// ==========================================
// 2. CONFIGURAÇÃO (Painel Admin & TicketConfig)
// ==========================================

// Buscar Configuração Atual (Para preencher o formulário TicketConfig.jsx)
// Essa função atende a rota GET /api/tickets/config
exports.getConfig = async (req, res) => {
    try {
        // Usa tenantId do usuário logado (Admin/Agent)
        ...
        if (result.rows.length > 0) {
            ...
            // Retorna padrão para não quebrar o frontend
            res.json({ 
                slug: '', 
...
// Alias para manter compatibilidade se a rota chamar getTenantConfig
exports.getTenantConfig = exports.getConfig; 

// Salvar Configuração (TicketConfig.jsx)
// Essa função atende a rota POST/PUT /api/tickets/config
exports.saveConfig = async (req, res) => {
    try {
        const tenantId = req.user.tenantId || req.user.tenant_id;
        const { slug, portal_title, primary_color, support_email, auto_assign, allow_guest_tickets } = req.body;

        // Verifica slug único globalmente (exceto para o próprio tenant)
        if (slug) {
            const check = await pool.query('SELECT tenant_id FROM helpdesk_config WHERE slug = $1 AND tenant_id != $2', [slug, tenantId]);
            if (check.rows.length > 0) return res.status(400).json({ error: 'Este endereço (slug) já está em uso.' });
        }
...
// Alias para updateConfig
exports.updateConfig = exports.saveConfig;

// ==========================================
// 3. CATEGORIAS E USUÁRIOS (Gestão)
// ==========================================

// Listar Categorias (Público ou Privado)
exports.getCategories = async (req, res) => {
    try {
        let tenantId = req.user?.tenantId || req.user?.tenant_id;
        
        // Se for público (via slug)
        ...
        // Adicionei coluna 'is_active' se existir, senão remove do WHERE
...
exports.createCategory = async (req, res) => {
    try {
        ...
        // Adicionei sla_hours que o TicketConfig pede
        const result = await pool.query(
...
// Listar Usuários Internos (Para Select de Atendentes)
// Atende a rota GET /api/tickets/users
exports.getUsers = async (req, res) => {
    try {
        const tenantId = req.user.tenantId || req.user.tenant_id;
        // Retorna apenas usuários do sistema (não clientes externos)
        const result = await pool.query(
...
// Listar Clientes de Suporte (Externos)
...
exports.getTickets = async (req, res) => {
    try {
        ...
        // Admin/Agent vê tudo. User/Employee/SupportUser vê apenas os seus.
        if (role !== 'admin' && role !== 'agent' && role !== 'manager') {
            query += ` AND t.requester_id = $2 AND t.requester_type = $3`;
...
exports.createTicket = async (req, res) => {
    try {
        const tenantId = req.user.tenantId || req.user.tenant_id;
        const userId = req.user.id;
        const role = req.user.role || 'user'; // support_user, employee, user
        const { subject, title, description, category_id, priority, category } = req.body;

        // Front pode mandar 'title' ou 'subject'
        const finalTitle = title || subject;

        // Se mandou nome da categoria (portal antigo), tenta achar ID
...
// Detalhes do Ticket (Com Mensagens)
exports.getTicketDetails = async (req, res) => {
    try {
        ...
        // 2. Segurança (Quem pode ver?)
        const isStaff = ['admin', 'agent', 'manager'].includes(role);
        if (!isStaff) {
            // Se não é staff, tem que ser o dono
            if (ticket.requester_id !== userId) return res.status(403).json({ error: 'Acesso negado' });
        }
        ...
        if (!isStaff) {
            msgQuery += ` AND m.is_internal_note = false`;
        }
...
exports.addMessage = async (req, res) => {
    try {
        ...
        const userId = req.user.id;
        const role = req.user.role;
        const { ticketId } = req.params; // Cuidado com nome do param nas rotas (:id vs :ticketId)
        const { message, is_internal } = req.body;

        // Verifica se ticket existe e pertence ao tenant
        const check = await pool.query('SELECT id FROM tickets WHERE id = $1 AND tenant_id = $2', [ticketId, tenantId]);
        if(check.rows.length === 0) return res.status(404).json({error: 'Ticket não encontrado'});
